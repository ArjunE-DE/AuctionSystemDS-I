...
def broadcast_state(no_timestamp = False, action={}):
...
def full_state_broadcast():
    while True:
        if LEADER == SERVER_ID:
            broadcast_state(False, {"Full state update from leader": SERVER_ID})
            for port in KNOWN_SERVERS:
                if port != SERVER_PORT:
                    send_full_state(port)
...
def process_command(command):
    ...
    if command["action"] == "register":
        ...
        broadcast_state(False, {"New user created": username})
        send_session_update()
    ...
    if command["action"] == "login":
        ...
        broadcast_state(False, {"User login": username})
        send_session_update()
...
def server_listener():
    ...
    while True:
        ...
        if msg.get("type") == "HS_ELECTION":
            ...
            leader_timestamp = msg.get("ts", None)
            if not leader_timestamp or leader_timestamp >= LAMPORT.read(): 
                STATE.update(msg.get("state", {}))
                SESSIONS.update(normalize_sessions(msg.get("sessions", {})))
                USERS.update(msg.get("users", {}))
                LAMPORT.update(leader_timestamp)
                leader_port = None
                for port, sid in KNOWN_SERVERS.items():
                    if sid == LEADER:
                        leader_port = port
            ...
            leader_timestamp = msg.get("ts", None)
            print(f"local timestamp: {LAMPORT.read()} received timestamp: {leader_timestamp}")
            if not leader_timestamp or leader_timestamp >= LAMPORT.read(): 
                old_leader = LEADER
